...
def test_level_1_splits_revenue_into_customers_frequency_and_aov() -> None:
    ...
    lever = compute_lever(data, WITHIN)
...
def test_an_inconclusive_lens_reports_null_not_false_for_the_alert() -> None:
    ...
    lever = compute_lever(data, [signal("revenue")])
...
def test_the_masked_shift_alert_needs_both_a_big_ratio_and_a_signal() -> None:
    """DIAGNOSE_DESIGN 1.2's case: revenue barely moves while the components
    move several times harder. The ratio alone is not enough - on a near-flat
    month the components always dwarf the net."""
    rows = [
        # October: 4 customers, 1 order each, 100 -> revenue 400
        *[row(date(2011, 10, 5), qty=1, price=100.0, customer=f"C{i}") for i in range(4)],
        # November: 1 customer, 4 orders, 100 -> revenue 400, total unchanged
        *[row(date(2011, 11, day), qty=1, price=100.0, customer="C0")
          for day in (5, 10, 20, 30)],
    ]
    data = run_data(rows)

    with_signal = compute_lever(data, [signal("active_customers")])
    without_signal = compute_lever(data, WITHIN)

    assert with_signal.masked_shift_alert is True
    assert without_signal.masked_shift_alert is False
    # Revenue did not move at all, so the ratio has no denominator - but the
    # alert is still decided, which is the whole point of this case.
    assert with_signal.gross_to_net is None
    assert "revenue did not move" in with_signal.reasons["gross_to_net"]
...
def test_the_alert_fires_above_the_threshold_and_not_below_it() -> None:
    """MASKED_GROSS_TO_NET had no coverage at all: the only masked-shift test
    was built so `gross_to_net` was None, which short-circuits before the
    threshold is ever consulted (3C doubt-review R4). This exercises the
    comparison itself, from both sides."""
    masked = run_data(_masked_rows())  # ratio 78.25, far above 3.0
    # Two customers become four, nothing else moves: the components and the
    # total move together, so the ratio is 1.0 and there is nothing masked.
    plain = run_data([
    ...
    loud = compute_lever(masked, [signal("active_customers")])
    quiet = compute_lever(plain, [signal("active_customers")])

    assert loud.gross_to_net is not None and loud.gross_to_net >= MASKED_GROSS_TO_NET
